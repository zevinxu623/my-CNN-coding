...
from mobilenet_v2.newmobilenet import mobilenet_v2
...
SAVE_PATH = '/home/xuzhiwen/pythonProject/mobilenet_v2/Log'
# save_path = os.path.join(SAVE_PATH, "log-inception-ours-4.txt")
# save_path = os.path.join(SAVE_PATH, "inception-gk-nonpre-1.txt")
save_path = os.path.join(SAVE_PATH, "mob-c-new-4.txt")
save_path2 = os.path.join(SAVE_PATH, "mob-our-mertic-2.txt")
...
optimizer = torch.optim.Adam(model.parameters(), lr=8.1e-5, weight_decay=0.96)
best_acc = 0.0
best_epoch = 0
MAX_EPOCH = 150
pre = [0.0, 0.0, 0.0]
rec = [0.0, 0.0, 0.0]
f1 = [0.0, 0.0, 0.0]
my_print("mob 0.75 512*512 batch_size = 8 , 0.4均分  8.1e-5 ", save_path)
# scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=(lambda epoch: 0.9 ** (epoch//8)))
...
for epoch in range(MAX_EPOCH):
    ...
    with torch.no_grad():
        ...
        #     plt.savefig("/home/xuzhiwen/pythonProject/right/metric/mob/gk-mob-pr-" + str(epoch + 1) + ".png")
        ...
        # # plt.show()
        # plt.savefig("/home/xuzhiwen/pythonProject/right/mob/confusion"+str(epoch+1)+".png", dpi=300)
        #
        # n1 = len(conf_matrix)
        # precision = []
        # recall = []
