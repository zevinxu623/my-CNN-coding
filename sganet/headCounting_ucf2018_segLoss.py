    ...
    for root, _, fnames in sorted(os.walk(d)):
            for fname in sorted(fnames):
                if has_file_allowed_extension(fname, extensions):
                    ...
                    label_path = os.path.join(head,'ground_truth_mat_resized_to_2048',fname[:-4]+'_ann.mat')
...
class ShanghaiTechDataset(Dataset):
    ...
    def __getitem__(self,idx):        
        ...
        fbs = generate_density_map(shape=image.shape,points=annPoints,f_sz=35,sigma=1)
        fbs = np.int32(fbs>0)
        ...
        if height < tsize[0] or width < tsize[1]:
            # padding zeros
            image = cv2.copyMakeBorder(image,0, np.maximum(0,tsize[0]-height), 0, np.maximum(0,tsize[1]-width),cv2.BORDER_CONSTANT, value = 0)
            positions = cv2.copyMakeBorder(positions,0, np.maximum(0,tsize[0]-height), 0, np.maximum(0,tsize[1]-width),cv2.BORDER_CONSTANT, value = 0)
            fbs = cv2.copyMakeBorder(fbs,0, np.maximum(0,tsize[0]-height),0, np.maximum(0,tsize[1]-width),cv2.BORDER_CONSTANT, value = 0)
